...
# 1) Grid‐search to find best NNAR hyperparameters
grid <- expand.grid(
  K_weekly = 1:3,              # max = floor(7/2) = 3
  K_annual = c(5, 10),         # 5 and 10 <= floor(365/2)
  size     = c(5, 10),
  decay    = c(0, 0.01),
  repeats  = c(10, 20)
)
...
results <- pmap_dfr(grid, function(K_weekly, K_annual, size, decay, repeats) {
  ...
  f_train <- fourier(y_train, K = c(K_weekly, K_annual))
  f_test  <- fourier(y_train, K = c(K_weekly, K_annual), h = h)
  ...
  fc   <- forecast(fit, h = h, xreg = f_test)
  mape <- accuracy(fc, y_test)["Test set", "MAPE"]
  
  tibble(K_weekly, K_annual, size, decay, repeats, MAPE = mape)
})

best <- results %>% arrange(MAPE) %>% slice_head(n = 1)

# 2) TBATS + humidity (baseline; assumes fc_tbats_hum exists)
fc_tb   <- fc_tbats_hum$mean

# 3) Fit NNAR with best hyperparameters
f_best_train <- fourier(y_train, K = c(best$K_weekly, best$K_annual))
f_best_test  <- fourier(y_train, K = c(best$K_weekly, best$K_annual), h = h)
...
fc_nn_best <- forecast(fit_nn_best, h = h, xreg = f_best_test)
...
# 4b) MAPE‐weighted
w_tb <- 1 / accuracy(fc_tbats_hum, y_test)["Test set", "MAPE"]
w_nn <- 1 / accuracy(fc_nn_best,   y_test)["Test set", "MAPE"]
w_tot <- w_tb + w_nn
ens_wt <- (w_tb / w_tot) * fc_tb + (w_nn / w_tot) * mean_nn

# 5) State‐space backstops
fc_stlf <- stlf(y_train, h = h, method = "ets")

ssm_fit <- StructTS(as.ts(y_train), type = "BSM")
fc_ssm  <- forecast(ssm_fit, h = h)

# 6) Extract accuracy metrics
acc_tb   <- accuracy(fc_tbats_hum, y_test)[1, c("ME", "RMSE", "MAE", "MAPE")]
...
acc_ssm  <- accuracy(fc_ssm,       y_test)[1, c("ME", "RMSE", "MAE", "MAPE")]
...
accuracy_advanced <- tibble(
  ...
  ME    = c(acc_tb["ME"],    acc_nn["ME"],    acc_avg["ME"],    acc_wt["ME"],    acc_stlf["ME"],    acc_ssm["ME"]),
  RMSE  = c(acc_tb["RMSE"],  acc_nn["RMSE"],  acc_avg["RMSE"],  acc_wt["RMSE"],  acc_stlf["RMSE"],  acc_ssm["RMSE"]),
  MAE   = c(acc_tb["MAE"],   acc_nn["MAE"],   acc_avg["MAE"],   acc_wt["MAE"],   acc_stlf["MAE"],   acc_ssm["MAE"]),
  MAPE  = c(acc_tb["MAPE"],  acc_nn["MAPE"],  acc_avg["MAPE"],  acc_wt["MAPE"],  acc_stlf["MAPE"],  acc_ssm["MAPE"])
) %>%
  arrange(MAPE)
...
accuracy_advanced %>%
  kable(
    caption = "Comparison of TBATS+Humidity, Tuned NNAR, Ensembles, and State‐Space Models",
    digits  = 2
  ) %>%
  kable_styling(full_width = FALSE)
```
...
```{r}
# 2. Assemble all point-forecast vectors into a named list
forecasts_advanced <- list(
  tbats_hum    = as.numeric(fc_tbats_hum$mean),
  nnar_tuned   = as.numeric(fc_nn_best$mean),
  ensemble_avg = ens_avg,
  ensemble_wt  = ens_wt,
  stl_ets      = as.numeric(fc_stlf$mean),
  bsm          = as.numeric(fc_ssm$mean)
)

# 3. Loop over each model and write the submission CSV
for(model in names(forecasts_advanced)) {
  out <- template %>%
    mutate(
      # ensure date column is "YYYY-MM-DD"
      !!date_col     := format(as.Date(.data[[date_col]]), "%Y-%m-%d"),
      # replace forecast column with this model's predictions
      !!forecast_col := forecasts_advanced[[model]]
    )
