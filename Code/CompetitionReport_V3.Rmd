...
# indices
n_train <- which(agg_daily$date == as.Date("2009-12-31"))
n_valid <- which(agg_daily$date == as.Date("2010-02-28"))
n_test  <- which(agg_daily$date == as.Date("2011-02-28"))

# training (2005-2009)
y_train <- window(y_full, end = c(2009, 365))
x_train <- x_full[1:n_train, ]

# validation (2010-01-01 → 2010-02-28)
y_valid <- window(y_full, start = c(2010, 1), end = c(2010, 59))
x_valid <- x_full[(n_train + 1):n_valid, ]

# test   (2011-01-01 → 2011-02-28)  – forecasts required by competition
x_test  <- x_full[(n_valid + 1):n_test, ]        # regressors known for 2010?
