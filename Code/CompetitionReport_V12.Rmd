...
```{r}
# Define your final forecast horizon and dates
future_dates <- seq(as.Date("2011-01-01"), as.Date("2011-02-28"), by = "day")
...
f_tr      <- fourier(y_val,      K = c(K_weekly, K_annual))
f_ts      <- fourier(y_val,      K = c(K_weekly, K_annual), h = h)

set.seed(123)
fc_nn_long  <- forecast(
  nnetar(y_val, xreg = f_tr, repeats = 20, MaxNWts = 5000),
  h    = h,
  xreg = f_ts
)
...
autoplot(y_val) +
  autolayer(fc_nn_long, series="NNAR + Four",PI=FALSE) +
