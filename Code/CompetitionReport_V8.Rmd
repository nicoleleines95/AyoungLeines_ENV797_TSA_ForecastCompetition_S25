...
fc_ses        <- forecast(es(y_train, model="ZZZ"), h = h, holdout=FALSE)      

# Basic structural model (BSM)
ss_fit        <- StructTS(y_train, type="BSM")
fc_bsm        <- forecast(ss_fit, h = h)                                        

# Hybrid ensemble of core methods
hyb           <- hybridModel(
                   y_train,
                   models = c("a","e","t","b","n"),   # ARIMA, ETS, Theta, TBATS, NNAR
                   weights = "equal"
                 )
fc_hybrid     <- forecast(hyb, h = h)
